...
public class FileUtil {
    ...
    public static String getFileSuffix(String filename, boolean isContainDot) {
        if (StringUtils.isBlank(filename) || filename.indexOf(CoCoMusicConstants.POINT_STR) == CoCoMusicConstants.MINUS_ONE_INT) {
            return StringUtils.EMPTY;
        }
        if (isContainDot) {
            return filename.substring(filename.lastIndexOf(CoCoMusicConstants.POINT_STR));
        } else {
            return filename.substring(filename.lastIndexOf(CoCoMusicConstants.POINT_STR) + CoCoMusicConstants.ONE_INT);
        }
    }
    ...
    public static String generateStoreFileChunkRealPath(String basePath, String identifier, Integer chunkNumber) {
        return new StringBuffer(basePath)
                ...
                .append(File.separator)
                .append(UUIDUtil.getUUID())
                .append(CoCoMusicConstants.COMMON_SEPARATOR)
                .append(chunkNumber)
                .toString();
    }
    ...
    public static void writeFile2OutputStream(FileInputStream fileInputStream, OutputStream outputStream, long length) throws IOException {
        ...
        fileChannel.transferTo(CoCoMusicConstants.ZERO_INT, length, writableByteChannel);
        outputStream.flush();
    ...
    public static void writeStream2StreamNormal(InputStream inputStream, OutputStream outputStream) throws IOException {
        ...
        int len;
        while ((len = inputStream.read(buffer)) != CoCoMusicConstants.MINUS_ONE_INT) {
            outputStream.write(buffer, CoCoMusicConstants.ZERO_INT, len);
        }
        outputStream.flush();
        inputStream.close();
        outputStream.close();
